...
primary_monitor=`i3-msg -t get_workspaces | jq '.[] | select(.visible==true).output'  | cut -d'"' -f2 | peco`
secondary_monitor=`i3-msg -t get_workspaces | jq '.[] | select(.visible==true).output'  | cut -d'"' -f2 | grep -v $primary_monitor`
